/**
 * Created by yangjin on 2017/3/22.
 */
/**
 * 水球图 wataerbubble
 */
...
(function($) {
    $.fn.waterbubble = function(options) {
        ...
        config.data=  config.data=config.data>1?1:config.data;
    ...
    Waterbubble.prototype = {
        ...
        _buildShape: function (canvas, config) {
            ...
            // }
            this._fillWater(ctx, r, 1, lineWidth, backColor, x, y, false);
        ...
        _drawText: function (ctx, textColor, font, radius, data, x, y, txt) {
            ...
            ctx.font = font ? font : ' ' + size + 'px Microsoft Yahei';

            txt = txt.length ? txt : data*100 + '%'

            var sy =y*2-y*2*data-15;
            if(sy<size+15){
                sy = y + size/2;
                textColor="#ffffff";
            }
